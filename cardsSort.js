/*
 Ниже приведен пример данных о маршруте - посадочные карточки.
 У карточек есть свойства пункта отправления и пункта назаначения.
 А также доп. информация о траспорте

     from: 'Saint-Petesburg',                - пункт отправления
     to: 'Moscow',                           - пунтк назначения
     type: 'train',                          - тип транспорта
     number: '78A',                          - номер транспорта
     addInfo: {                              - доп инофрмация об отрезке
                                             - объет доп инофрмации не зафиксирован в него можно передать
                                               неограниченное количество свойств.
         'Gate': 22,
         'Seat': '3A',
  ...
  Сам алгоритм очень прост.
    1. Создаем объект ticketBook - плоский вариант связанного списка.
    2. Находим начальную точку маршрута
    3. Проходимся по связаному списку от начальной точки и пока в списке не будет найдена
       свойство с текущим месторасположением.
*/
const cards = [
    {
